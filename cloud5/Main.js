...
function onClick(e) {
	...
	//画面 1->2 ... 4->5 ... 11->12 ->20
 	if (0 <= x && x < imgsize*4 && 0 <= y && y < imgsize*4 && screen%20 >= 1 && screen%20 <= 12) {

 		context.clearRect(0, 0, 1500, 1000);
        let x = e.offsetX;
        let y = e.offsetY;
        let number = Math.floor(y / imgsize) * 4 + Math.floor(x / imgsize) + 1;
        answer.push(number);
...
function next() {
	...
	//画面更新0->1 ... 3->4 ... 11->12
 	...
 	if (screen == 100) {
		...
		fetch("https://script.google.com/macros/s/AKfycbzQ4I2FiA1Z8lEhKIGu5mbG_XBQceW_MjFOGiv9JogxG6mnx4oFhH4dAJuqOb4pDKnG/exec" , {
			method: "POST",
			body: json_data,
			mode: 'no-cors',
		...
		let sentences = [
			"これで実験は終了です。お疲れ様でした。",
			"確認コードは、「pipo」になります。ランサーズ上の実験後アンケートに回答してください。",
...
let data =[
	...
	//1-index
	["耳に髪がかかっていない人", [11, 11, 13, 7, 13, 1, 14, 2, 2, 11, 11, 15]],

	["モンゴロイドの女性", [3, 3, 13, 13, 16, 14, 2, 15, 1, 13, 3, 7]],

	["背景が緑", [3, 4, 11, 9, 3, 11, 11, 2, 10, 12, 15, 5]],

	["幼い女の子", [4, 5, 2, 4, 10, 6, 7, 12, 9, 7, 4, 14]]

	];

//練習用データ
let p_data = [
	["前髪で額が隠れている人", [3, 3, 5, 14, 11, 15, 3, 6, 13, 9, 2, 13]],
	];
...
function name_input() {
	...
	//画面更新-1->0
 	if (screen == -1 && name.value.length>0) {
		...
		context.font = "24px sans-serif";
		let sentences = [
			"16枚の顔画像が画面に表示され、1枚のみがある条件を満たします。",
