	...
	//画面更新0->1 ... 3->4
 	...
 	if (screen%20 == 12 & screen<240) {
 		...
 		if (confirm("推測した条件は「"+condition.value+"」でいいですか？")==true) {
 			...
 			context.clearRect(0, 0, 1500, 700);
			context.font = "24px sans-serif";
			let sentences = ["正解の条件 : "+data[pass][0],
				"認証正解数 "+point+"/8"]
			for (let i = 0; i < sentences.length; i++) {
				context.fillText(sentences[i], 0, 24*i+24);
				}
 	...
 	if (screen == 240) {
	    ...
	    //送信データ
	    var dt = [name.value, points, answer, conditions, all_time];
	    var json_data = JSON.stringify(dt);

		fetch("https://script.google.com/macros/s/AKfycbx0FDo2t7jikYDReEz_rpex7OoD5RG92g6yvScE8vpmNwqx5sRU1OJRiIK5MXZPQRSTEg/exec" , {
			method: "POST",
			body: json_data,
			mode: 'no-cors',
	...
	//画面更新-1->0
 	if (screen == -1 && name.value.length>0) {
		...
		let sentences = [
			...
			"ただし赤枠は画像表示から1.7秒後から0.3秒間のみ表示されます。画像表示から2.0秒経過すると自動的に次の画像が表示されます。",	
			"その後、条件を推測して、残り8例のパスワードの認証を行ってください。パスワード認証では時間制限はありません",
			"画面上部の「次に進む」をクリックすると実験が開始します。",
			"なお実験は12種類のパスワードで実施します。"]
